...
export default function TestimonialSection() {
  ...
  const sliderData = testimonialsData.length
  ...
  const fetchTestimonials = async () => {
    try {
      const res = await ApiGet("/admin/testimonial");
      ...
      if (data.length > 0) {
        const mapped = data.map((item, index) => ({
          id: item._id || index,
          name: item.name,
          role: item.city || item.role || "Customer",
          image: item.image,
          rating: item.rating || 5,
          product: item.productName || "Gawdee Product",
          short: item.short || item.review?.slice(0, 100),
          full: item.full || item.fullReview || "",
          tags: item.tags || [],
        }));
  ...
  return (
    <>
      <section className="relative pt-[60px] pb-[40px] lg:py-28 overflow-hidden ">
        ...
        <div className=" mx-auto px-4 lg:px-6">
          <div className="text-center max-w-[760px] mx-auto mb-[20px] lg:mb-14">
            <motion.div
              ...
              className="absolute left-[0px] right-0 top-[20px] lg:top-[40px]  md:block text-[42px] lg:text-[130px] font-semibold uppercase text-transparent [-webkit-text-stroke:2px_rgba(60,80,40,0.25)] leading-none select-none pointer-events-none [mask-image:linear-gradient(to_bottom,black_10%,transparent)]"
            >
              Testimonials
            </motion.div>
            <h2 className="lg:mt-4 text-[24px] md:text-[50px] leading-[1.05] font-semibold text-[#1d2b1f]">
              Loved By Families Who
              <span className="block text-[#8c7440]">Choose Purity Daily</span>
            </h2>
            <p className=" mt-[4px] lg:mt-5 lg:text-[15px] text-[12px] text-[#667266]  leading-[19px] lg:leading-7">
                ...
                <AnimatePresence mode="popLayout">
                  {sliderData.map((item, index) => (
                    <motion.button
                      ...
                      className="text-left mr-4 lg:mr-6 min-w-[120px] w-[300px] lg:w-[400px] relative rounded-[20px] lg:rounded-[28px] bg-white/80 backdrop-blur-xl border border-[#e9e2d4] p-4 lg:p-5  shadow-lg hover:-translate-y-1 transition-all duration-300"
                    >
                      <div className="absolute right-6 top-3 w-10 h-10 rounded-full bg-[#0c776b]/30 flex lg:hidden items-center justify-center text-[#0c776b]">
                        <Quote size={17} />
                      </div>
                       <div className="absolute   hidden right-6 top-6 w-12 h-12 rounded-full bg-[#0c776b]/30 lg:flex items-center justify-center text-[#0c776b]">
                        <Quote size={20} />
                      </div>
                      ...
                      <div className="lg:mt-6 mt-2 flex items-center justify-between">
                        ...
                        <span className="w-8 h-8 rounded-full bg-[#0c776b] text-white flex items-center justify-center">
